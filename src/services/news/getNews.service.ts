...
class GetNewsService {
  public async getNews (lan: Languages, pageSize: Number = 10, page: Number = 1): Promise<Responses> {
    ...
    await this.processNews(lan, pageSize, page)
      ...
      .then((res) => {
        ...
        responses.answer = res
        return res
      })
    return responses
  }

  private async processNews (lan: Languages, pageSize: Number = 10, page: Number = 1): Promise< NewsResponse> {
    const res: NewsResponse | boolean = await this.fetchNews(lan, pageSize, page)
      .then(async (response) => {
        if (!response.ok) {
  ...
  private async fetchNews (lan: Languages, pageSize: Number = 10, page: Number = 1): Promise<Response> {
    const query = queryLanguages[lan]
    const route = `${msRoutes.news_ms.route}?${query}&pageSize=${pageSize.toString()}&page=${page.toString()}&apiKey=${msRoutes.news_ms.key}`
    console.log(route)
